...
/* GET /api/logs → JSON 배열 ["20260829_162958.log", ...] */
static void serve_log_list(SOCKET s) {
    ...
    int first = 1;
    /* 이름 목록 수집 후 역순 정렬은 생략 — FindFirstFile은 순서 보장 없음.
     * 운영 환경에서는 파일이 타임스탬프 이름이라 lexicographic 역순 = 최신순. */
    if (hFind != INVALID_HANDLE_VALUE) {
        do {
            if (fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) continue;
            char esc[128];
            json_str(fd.cFileName, esc, sizeof(esc));
